...
.chat-item-active { background: #2f2f2f !important; color: #ececec !important; }
...
# ── Config ────────────────────────────────────────────────────────────────────
API_KEY = "sua_key_aqui"  # ← coloca sua key aqui
...
if not memoria["nome"]:
    ...
    with st.sidebar:
        ...
        for chat_id in sorted(chats.keys(), reverse=True):
            titulo = chats[chat_id]["titulo"]
            ativo = "chat-item-active" if chat_id == st.session_state.chat_atual else ""
        ...
        # Deletar chat atual
        if st.session_state.chat_atual and st.session_state.chat_atual in chats:
    ...
    if not st.session_state.chat_atual or st.session_state.chat_atual not in st.session_state.chats:
        ...
        <div class="chat-wrapper">
            <div class="welcome">
                ...
                <p>Clique em <b>Novo chat</b> para começar uma conversa.</p>
            </div>
        </div>
        """, unsafe_allow_html=True)
