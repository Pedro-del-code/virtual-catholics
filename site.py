...
if not st.session_state.intro_exibida:
    ...
    _intro_parts = [
        ...
        "#vci{position:fixed;inset:0;z-index:999999;background:linear-gradient(150deg,#f0c000 0%,#e8b800 35%,#fffbe8 65%,#ffffff 100%);display:flex;flex-direction:column;align-items:center;justify-content:center;cursor:pointer;}",
        "#vcr{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;opacity:0;pointer-events:none;}",
        "#vcr svg{width:700px;height:700px;}",
        "#vcli{width:220px;height:220px;object-fit:contain;opacity:0;}",
        "#vct{font-family:'Cinzel',serif;font-size:0px;font-weight:900;color:#5a3800;letter-spacing:0.25em;text-transform:uppercase;opacity:0;white-space:nowrap;text-shadow:0 2px 12px rgba(255,255,255,0.7);}",
        "#vcg{font-family:'Cormorant Garamond',serif;font-style:italic;font-size:0px;color:rgba(80,50,0,0.65);letter-spacing:0.3em;opacity:0;}",
        "#vcl{width:0px;height:1px;background:linear-gradient(90deg,transparent,#7a5200,transparent);opacity:0;margin:0 auto;}",
        "#vcs{position:absolute;bottom:28px;font-family:'Cinzel',serif;font-size:9px;color:rgba(80,50,0,0.4);letter-spacing:0.35em;text-transform:uppercase;animation:vcb 2.5s ease-in-out infinite;}",
        "@keyframes vcb{0%,100%{opacity:0.3;}50%{opacity:0.7;}}",
        "#vci.vcf{animation:vcfo 1.4s ease-in forwards;}",
        "@keyframes vcfo{to{opacity:0;pointer-events:none;}}",
        "</style>",
        "<div id='vci' onclick='vcSk()'>",
        "<div id='vcr'><svg viewBox='0 0 700 700' xmlns='http://www.w3.org/2000/svg'>",
        "<defs><radialGradient id='rg' cx='50%' cy='50%' r='50%'>",
        "<stop offset='0%' stop-color='#ffffff' stop-opacity='0.5'/>",
        "<stop offset='100%' stop-color='#f0c000' stop-opacity='0'/></radialGradient></defs>",
        "<g transform='translate(350,350)'>",
        "<line x1='0' y1='0' x2='0' y2='-320' stroke='white' stroke-width='22' opacity='0.35'/>",
        "<line x1='0' y1='0' x2='150' y2='-280' stroke='white' stroke-width='14' opacity='0.25'/>",
        "<line x1='0' y1='0' x2='280' y2='-150' stroke='white' stroke-width='10' opacity='0.2'/>",
        "<line x1='0' y1='0' x2='320' y2='0' stroke='white' stroke-width='16' opacity='0.28'/>",
        "<line x1='0' y1='0' x2='280' y2='150' stroke='white' stroke-width='10' opacity='0.2'/>",
        "<line x1='0' y1='0' x2='150' y2='280' stroke='white' stroke-width='14' opacity='0.25'/>",
        "<line x1='0' y1='0' x2='0' y2='320' stroke='white' stroke-width='22' opacity='0.35'/>",
        "<line x1='0' y1='0' x2='-150' y2='280' stroke='white' stroke-width='14' opacity='0.25'/>",
        ...
        "<line x1='0' y1='0' x2='-280' y2='-150' stroke='white' stroke-width='10' opacity='0.2'/>",
        "<line x1='0' y1='0' x2='-150' y2='-280' stroke='white' stroke-width='14' opacity='0.25'/>",
        "</g><circle cx='350' cy='350' r='140' fill='url(#rg)'/></svg></div>",
        "<div style='display:flex;flex-direction:column;align-items:center;gap:20px;position:relative;z-index:2;'>",
    ]
    _intro_img = f"<img id='vcli' src='data:image/png;base64,{_LOGO_B64}' alt='logo'/>"
    _intro_end = [
        "<div id='vcl'></div>",
        "<div id='vct'>Virtual Catholics</div>",
        "<div id='vcg'>Companheiro Espiritual</div>",
        "</div>",
        "<div id='vcs'>toque para pular</div>",
        "</div>",
        "<script>",
        "var _vs=false;",
        "function pB(){try{var c=new(window.AudioContext||window.webkitAudioContext)();",
        "function b(f,t,d,v){var o1=c.createOscillator(),o2=c.createOscillator(),g=c.createGain();",
        "o1.type=o2.type='sine';o1.frequency.value=f;o2.frequency.value=f*2.756;",
        "g.gain.setValueAtTime(0,c.currentTime+t);g.gain.linearRampToValueAtTime(v,c.currentTime+t+0.01);",
        "g.gain.exponentialRampToValueAtTime(0.001,c.currentTime+t+d);",
        "o1.connect(g);o2.connect(g);g.connect(c.destination);",
        "o1.start(c.currentTime+t);o1.stop(c.currentTime+t+d);",
        "o2.start(c.currentTime+t);o2.stop(c.currentTime+t+d);}",
        "function dr(f,v){var o=c.createOscillator(),g=c.createGain();o.type='sine';o.frequency.value=f;",
        "g.gain.setValueAtTime(0,c.currentTime);g.gain.linearRampToValueAtTime(v,c.currentTime+2);",
        "g.gain.linearRampToValueAtTime(0,c.currentTime+9);o.connect(g);g.connect(c.destination);",
        "o.start();o.stop(c.currentTime+9);}",
        "b(523.25,0.1,4.5,0.13);b(659.25,1.2,3.5,0.10);b(783.99,2.3,3.0,0.12);",
        "b(1046.5,3.5,5.0,0.08);b(523.25,5.0,4.0,0.09);dr(130.81,0.05);dr(196,0.04);",
        "}catch(e){}}",
        "var sl=ms=>new Promise(r=>setTimeout(r,_vs?0:ms));",
        "function an(d,cb,e){e=e||(t=>1-Math.pow(1-t,3));return new Promise(r=>{if(_vs){cb(1);r();return;}",
        "var s=performance.now();function st(n){if(_vs){cb(1);r();return;}",
        "var p=Math.min((n-s)/d,1);cb(e(p));if(p<1)requestAnimationFrame(st);else r();}",
        "requestAnimationFrame(st);})}",
        "async function ri(){pB();",
        "var im=document.getElementById('vcli'),ra=document.getElementById('vcr'),",
        "ti=document.getElementById('vct'),tg=document.getElementById('vcg'),li=document.getElementById('vcl');",
        "await an(2800,p=>{im.style.opacity=p;im.style.filter='brightness('+(0.5+p*0.5)+') saturate('+(0.4+p*0.6)+')';});",
        "await sl(300);await an(1000,p=>{ra.style.opacity=p;});await sl(200);",
        "await an(700,p=>{var pk=p<0.5?p*2:(1-p)*2;im.style.filter='brightness('+(1+pk*0.6)+') drop-shadow(0 0 '+(pk*35)+'px rgba(255,220,80,0.9))';});",
        "await sl(150);await an(700,p=>{li.style.width=(p*240)+'px';li.style.opacity=p;});",
        "await sl(150);await an(900,p=>{ti.style.fontSize=(p*20)+'px';ti.style.opacity=p;ti.style.letterSpacing=(0.5-p*0.25)+'em';});",
        "await sl(250);await an(700,p=>{tg.style.fontSize=(p*12)+'px';tg.style.opacity=p;});",
        "await sl(2000);if(!_vs)vcEn();}",
        "function vcEn(){_vs=true;var el=document.getElementById('vci');el.classList.add('vcf');setTimeout(()=>el.style.display='none',1400);}",
        "function vcSk(){vcEn();}",
        "ri();",
        "</script>",
    ]
    st.markdown("".join(_intro_parts) + _intro_img + "".join(_intro_end), unsafe_allow_html=True)
# ─────────────────────────────────────────────────────────────────────────────
...
if not st.session_state.logado:
    ...
    with col2:
        st.markdown(
            """<style>
@import url('https://fonts.googleapis.com/css2?family=Cinzel:wght@400;700;900&family=Cormorant+Garamond:ital,wght@0,300;1,300&display=swap');
.vc-wrap{background:rgba(255,255,255,0.45);border:1px solid rgba(200,160,0,0.35);border-radius:20px;padding:36px 28px 28px 28px;box-shadow:0 8px 48px rgba(180,130,0,0.18);backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);}
.vc-title{font-family:'Cinzel',serif!important;font-size:1.4rem!important;font-weight:900!important;color:#5a3800!important;letter-spacing:0.2em!important;text-align:center;margin:10px 0 4px 0;text-shadow:0 2px 12px rgba(255,255,255,0.8);}
.vc-sub{font-family:'Cormorant Garamond',serif!important;font-style:italic;font-size:0.85rem;color:rgba(80,50,0,0.65);letter-spacing:0.25em;text-align:center;margin-bottom:16px;}
.vc-div{width:100%;height:1px;background:linear-gradient(90deg,transparent,rgba(180,130,0,0.5),transparent);margin:0 0 16px 0;}
.stTextInput>div>div>input{background:rgba(255,255,255,0.75)!important;border:1.5px solid rgba(180,130,0,0.4)!important;border-radius:10px!important;color:#4a3000!important;font-family:'Cinzel',serif!important;font-size:0.8rem!important;letter-spacing:0.05em!important;padding:0.8rem 1rem!important;-webkit-text-fill-color:#4a3000!important;}
.stTextInput>div>div>input:focus{border-color:#c8a96e!important;box-shadow:0 0 0 3px rgba(200,169,110,0.2)!important;}
.stTextInput>div>div>input::placeholder{color:rgba(100,70,10,0.5)!important;-webkit-text-fill-color:rgba(100,70,10,0.5)!important;}
.stFormSubmitButton>button{background:linear-gradient(135deg,#c8a96e,#a07830)!important;border:none!important;border-radius:10px!important;color:#fff!important;font-family:'Cinzel',serif!important;font-size:0.8rem!important;font-weight:700!important;letter-spacing:0.2em!important;padding:0.85rem!important;box-shadow:0 4px 18px rgba(160,120,40,0.35)!important;}
.stFormSubmitButton>button:hover{opacity:0.88!important;}
.stButton>button{background:rgba(255,255,255,0.5)!important;border:1.5px solid rgba(180,130,0,0.4)!important;border-radius:10px!important;color:#5a3800!important;font-family:'Cinzel',serif!important;font-size:0.75rem!important;font-weight:600!important;letter-spacing:0.15em!important;min-height:42px!important;}
.stButton>button:hover{background:rgba(200,169,110,0.2)!important;border-color:#c8a96e!important;}
</style>""" + f'''<div class="vc-wrap">
<div style="text-align:center"><img src="data:image/png;base64,{_LOGO_B64}" style="width:88px;height:88px;object-fit:contain;filter:drop-shadow(0 4px 16px rgba(200,150,0,0.3));"/></div>
<div class="vc-title">Virtual Catholics</div>
<div class="vc-sub">Companheiro Espiritual</div>
